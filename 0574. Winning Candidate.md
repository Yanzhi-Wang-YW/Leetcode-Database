...
def winning_candidate(candidate: pd.DataFrame, vote: pd.DataFrame) -> pd.DataFrame: 
    output = vote.merge(candidate, how='inner', left_on='candidateId', right_on='id') 
    output = output.groupby('name').size().reset_index(name='cnt')
    output = output.sort_values('cnt', ascending=False)
