...
def find_students(departments: pd.DataFrame, students: pd.DataFrame) -> pd.DataFrame: 
    output = students.merge(departments, how='left', left_on='department_id', right_on='id', indicator=True) 
    output = output.loc[output['_merge'] == 'left_only'] 
    output = output[['id_x', 'name_x']] 
    output = output.rename(columns={'id_x': 'id', 'name_x': 'name'})   
    return output  
``` 
