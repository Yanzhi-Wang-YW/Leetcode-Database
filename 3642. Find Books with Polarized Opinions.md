...
def find_polarized_books(books: pd.DataFrame, reading_sessions: pd.DataFrame) -> pd.DataFrame:
    # A book has polarized opinions if it has at least one rating ≥ 4 and at least one rating ≤ 2 
    polarized_opinions = reading_sessions[['book_id', 'session_rating']].copy() 
    polarized_opinions['rating_more_than_four'] = np.where(
        polarized_opinions['session_rating'] >= 4, 
        1, 
        0
    )
    polarized_opinions['rating_less_than_two'] = np.where(
        polarized_opinions['session_rating'] <= 2, 
        1, 
        0
    )
    polarized_opinions = polarized_opinions.groupby('book_id').agg(
        sum_four=('rating_more_than_four', 'sum'), 
        sum_two=('rating_less_than_two', 'sum') 
    ).reset_index() 
    polarized_opinions = polarized_opinions.loc[
        (polarized_opinions['sum_four'] >= 1) & 
        (polarized_opinions['sum_two'] >= 1), 
        ['book_id']
    ] 
    # Only consider books that have at least 5 reading sessions 
    total_reading_sessions = reading_sessions[['book_id', 'session_id']].copy() 
    total_reading_sessions = total_reading_sessions.groupby('book_id').agg(
        cnt=('session_id', 'count')      
    ).reset_index() 
    total_reading_sessions = total_reading_sessions.loc[total_reading_sessions['cnt'] >= 5, ['book_id']] 
    # Calculate the rating spread as (highest_rating - lowest_rating) 
    rating_spread = reading_sessions[['book_id', 'session_rating']].copy() 
    rating_spread = rating_spread.groupby('book_id').agg(
        max_rating=('session_rating', 'max'), 
        min_rating=('session_rating', 'min')
    ).reset_index()
    rating_spread['rating_spread'] = rating_spread['max_rating'] - rating_spread['min_rating'] 
    rating_spread = rating_spread[['book_id', 'rating_spread']]          
    # Calculate the polarization score 
    polarization_score = reading_sessions[['book_id', 'session_rating']].copy() 
    polarization_score['rating_more_than_four'] = np.where(
        polarization_score['session_rating'] >= 4, 
        1, 
        0
    )
    polarization_score['rating_less_than_two'] = np.where(
        polarization_score['session_rating'] <= 2, 
        1, 
        0
    ) 
    polarization_score['has_extreme_ratings'] = np.where(
        (polarization_score['rating_more_than_four'] == 1) | 
        (polarization_score['rating_less_than_two'] == 1), 
        1, 
        0   
    ) 
    polarization_score = polarization_score.groupby('book_id').agg(
        total_extreme_ratings=('has_extreme_ratings', 'sum'), 
        total_session=('book_id', 'count') 
    ).reset_index() 
    polarization_score['polarization_score'] = (polarization_score['total_extreme_ratings'] / polarization_score['total_session'] * 100 + 0.5).astype(int) / 100.0     
    polarization_score = polarization_score.loc[polarization_score['polarization_score'] >= 0.6, ['book_id', 'polarization_score']] 
    # output 
    output = polarized_opinions.merge(total_reading_sessions, how='inner', on='book_id')    
    output = output.merge(rating_spread, how='inner', on='book_id') 
    output = output.merge(polarization_score, how='inner', on='book_id') 
    output = books.merge(output, how='inner', on='book_id') 
    output = output.sort_values(['polarization_score', 'title'], ascending=[False, False ]) 
    return output
```
