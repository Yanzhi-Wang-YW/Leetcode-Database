...
def shortest_distance(point2_d: pd.DataFrame) -> pd.DataFrame: 
    output = point2_d.merge(point2_d,how='cross') 
    output = output.rename(columns={'x_x': 'x1', 'y_x': 'y1', 'x_y': 'x2', 'y_y': 'y2'}) 
    output['shortest'] = (((output['x2'] - output['x1'])**2 + (output['y2'] - output['y1'])**2) ** 0.5 * 100 + 0.5).astype(int) / 100.0 
    output = output[['shortest']] 
    output = output.loc[output['shortest'] != 0]  
    output = output.sort_values(['shortest'], ascending=True) 
