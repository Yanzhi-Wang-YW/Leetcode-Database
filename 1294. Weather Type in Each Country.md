...
def weather_type(countries: pd.DataFrame, weather: pd.DataFrame) -> pd.DataFrame:  
    ...
    select_month = '2019-11' 
    weather = weather.loc[weather['day'].dt.strftime('%Y-%m') == select_month] 
    weather = weather.groupby('country_id').agg(
        avg_weather_state=('weather_state', 'mean') 
    ).reset_index() 
    weather['weather_type'] = np.where(
        weather['avg_weather_state'] <= 15, 
        'Cold', 
        np.where(
            weather['avg_weather_state'] >= 25, 
            'Hot', 
            'Warm' 
        )
    ) 
    output = weather.merge(countries, how='inner', on='country_id') 
    output = output[['country_name', 'weather_type']]    
    return output 
```   
