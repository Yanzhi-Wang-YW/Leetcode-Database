...
def find_zombie_sessions(app_events: pd.DataFrame) -> pd.DataFrame:
    # The session duration is more than 30 minutes 
    session_duration = app_events[['session_id', 'user_id', 'event_timestamp']].copy()
    session_duration = session_duration.groupby(['session_id', 'user_id']).agg(
        max_time=('event_timestamp', 'max'), 
        min_time=('event_timestamp', 'min')
    ).reset_index()
    session_duration['max_time'] = pd.to_datetime(session_duration['max_time'])
    session_duration['min_time'] = pd.to_datetime(session_duration['min_time'])
    session_duration['session_duration_minutes'] = (session_duration['max_time'] - session_duration['min_time']).dt.total_seconds() / 60 
    session_duration = session_duration.loc[
        session_duration['session_duration_minutes'] >= 30, 
        ['session_id', 'user_id', 'session_duration_minutes']
    ] 
    # Has at least 5 scroll events 
    scroll_events = app_events[['session_id', 'user_id', 'event_type']] .copy()
    scroll_events = scroll_events.loc[scroll_events['event_type'].eq('scroll')]
    scroll_events = scroll_events.groupby(['session_id', 'user_id']).agg(
        scroll_count=('event_type', 'count')        
    ).reset_index() 
    scroll_events = scroll_events.loc[scroll_events['scroll_count'] >= 5] 
    # The click-to-scroll ratio is less than 0.20 
    click_to_scroll = app_events[['session_id', 'user_id', 'event_type']].copy() 
    click_to_scroll['check_click'] = np.where(
        click_to_scroll['event_type'].eq('click'), 
        1, 
        0
    )
    click_to_scroll['check_scroll'] = np.where(
        click_to_scroll['event_type'].eq('scroll'), 
        1, 
        0
    )
    click_to_scroll = click_to_scroll.groupby(['session_id', 'user_id']).agg(
        sum_click=('check_click', 'sum'), 
        sum_scroll=('check_scroll', 'sum')
    ).reset_index() 
    click_to_scroll['percent'] = click_to_scroll['sum_click'] / click_to_scroll['sum_scroll'] 
    click_to_scroll = click_to_scroll.loc[click_to_scroll['percent'] < 0.2, ['session_id', 'user_id']] 
    # No purchases were made during the session 
    has_purchases = app_events[['session_id', 'user_id', 'event_type']].copy()
    has_purchases = has_purchases.loc[has_purchases['event_type'].eq('purchase')] 
    has_purchases = has_purchases.groupby(['session_id', 'user_id']).agg(
        cnt=('session_id', 'count')
    ).reset_index() 
    has_purchases = has_purchases[['session_id', 'user_id']] 
    no_purchases = app_events.merge(has_purchases, how='left', on=['session_id', 'user_id'], indicator=True) 
    no_purchases = no_purchases.loc[no_purchases['_merge'].eq('left_only'), ['session_id', 'user_id']]
    no_purchases = no_purchases.groupby(['session_id', 'user_id']).agg(
        cnt=('user_id', 'count')
    ).reset_index()
    no_purchases = no_purchases[['session_id', 'user_id']] 
    # output 
    output = session_duration.merge(scroll_events, how='inner', on=['session_id', 'user_id']) 
    output = output.merge(click_to_scroll, how='inner', on=['session_id', 'user_id']) 
    output = output.merge(no_purchases, how='inner', on=['session_id', 'user_id']) 
    output = output.sort_values(['scroll_count', 'session_id'], ascending=[False, True])    
    return output     
```
