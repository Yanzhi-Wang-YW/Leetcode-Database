...
def acceptance_rate(friend_request: pd.DataFrame, request_accepted: pd.DataFrame) -> pd.DataFrame:
    # count request 
    count_request = friend_request.groupby(['sender_id', 'send_to_id']).size().reset_index(name='cnt') 
    request_val = count_request['sender_id'].size 
    # count accept 
    count_accept = request_accepted.groupby(['requester_id', 'accepter_id']).size().reset_index(name='cnt')
    accept_val = count_accept['requester_id'].size 
    if request_val == 0: 
        return pd.DataFrame({'accept_rate': [0]})        
    return pd.DataFrame({'accept_rate': [int(accept_val / request_val * 100 + 0.5) / 100.0]})  
``` 
