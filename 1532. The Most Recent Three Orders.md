...
def recent_three_orders(customers: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    ...
    output = output.rename(columns={'name': 'customer_name'})
    output = output.sort_values(['customer_name', 'customer_id', 'order_date'], ascending=[True, True, False])
    output['row_num'] = output.groupby('customer_id').cumcount() + 1 
    output = output.loc[output['row_num'] <= 3, ['customer_name', 'customer_id', 'order_id', 'order_date']]
    return output
```
