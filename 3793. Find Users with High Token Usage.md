...
def find_users_with_high_tokens(prompts: pd.DataFrame) -> pd.DataFrame:
    # submitted at least 3 prompts
    total_prompt_per_user = prompts.groupby('user_id').agg(prompt_count=('prompt', 'count')).reset_index()
    filter_prompt = total_prompt_per_user.loc[total_prompt_per_user['prompt_count'] >= 3]  
    # submitted at least one prompt with tokens greater than their own average token usage  
    avg_token_per_user = prompts.groupby(['user_id']).agg(
        sum_token=('tokens', 'sum'), 
        count_prompt=('prompt', 'count')
    ).reset_index() 
    avg_token_per_user['avg_token'] = (avg_token_per_user['sum_token'] / avg_token_per_user['count_prompt'] * 100 + 0.5).astype(int) / 100
    avg_token_per_user = avg_token_per_user[['user_id', 'avg_token']]  
    filter_avg_token = prompts.merge(avg_token_per_user, how='inner', on='user_id')
    filter_avg_token = filter_avg_token[['user_id', 'tokens', 'avg_token']] 
    filter_avg_token = filter_avg_token.loc[filter_avg_token['tokens'] > filter_avg_token['avg_token']]
    filter_avg_token = filter_avg_token.groupby('user_id').agg(avg_tokens=('avg_token', 'first')).reset_index() 
    output = filter_prompt.merge(filter_avg_token, how='inner', on='user_id') 
    output = output.sort_values(['avg_tokens', 'user_id'], ascending=[False, True])  
    return output 
```

![alt text](AlteryxScreenshot/3793.png)
