...
def count_students(student: pd.DataFrame, department: pd.DataFrame) -> pd.DataFrame: 
    output = department.merge(student, how='left', on='dept_id') 
    output = output.groupby('dept_name').agg(
      student_number=('student_id', 'count')  
    ).reset_index() 
    output = output.sort_values(['student_number', 'dept_name'], ascending=[False, True])  
    return output  
``` 
