<div class="elfjS" data-track-load="description_content"><p>Table: <code>Visits</code></p>
...
def draw_chart(visits: pd.DataFrame, transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    trans_counts = merged.groupby(['user_id', 'visit_date']).agg(
        transactions_count=('amount', 'count') 
    ).reset_index() 
    output = trans_counts.groupby('transactions_count').size().reset_index(name='visits_count') 
    min_val = 0 
    max_val = output['transactions_count'].max() 
    mapping_Table = pd.DataFrame({'transactions_count': range(min_val, max_val + 1)}) 
    output = mapping_Table.merge(output, how='left', on='transactions_count') 
    output['visits_count'] = output['visits_count'].fillna(value=0) 
    output = output.sort_values(['transactions_count'], ascending=[True])           
    return output
```
