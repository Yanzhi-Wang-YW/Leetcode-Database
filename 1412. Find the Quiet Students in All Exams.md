...
![alt text](/FabricScreenshot/1412_stepoverview.png)
...
Keep only the rows where a student’s score equals the `min_score` or `max_score` for that exam.  

#### Configuration
- Filter rows with this logic: `[score] = [min_score] OR [score] = [max_score]`. 
...
Goal: keep rows from **Student Take Exam** where `student_id` is *not found* in the noisy-student list. 
...
Goal: return just the list of student ids that are “not noisy”. 
